...
function parsePrice(text){
  ...
  for(const raw of String(text||"").split(/\r?\n/)){
    ...
    // Main price must be taken BEFORE Naik/Turun, otherwise Rp20 change could be read as price.
    const mainPart=line.split(/\b(?:Naik|Turun)\b/i)[0];
    const price=closed ? null : priceNumberToken(mainPart);

    if(!closed && !(price>0)){
      // Not a recognized price line; do not create a bad row.
      continue;
    }

    let supplierExpr=mainPart
...
function normalizeMasterUnitCode(value){
  return String(value||"")
    .trim()
    .toUpperCase()
    .replace(/\s+/g,"-")
...
function setMasterUnitFormMessage(message,type=""){
  const el=$("masterUnitFormMsg");
  if(!el) return;
  el.textContent=message||"";
  el.className="unit-form-msg"+(type?` ${type}`:"");
}

function openMasterUnitModal(unitId=null){
  const modal=$("masterUnitModal");
  if(!modal) return;

  const unit=unitId!=null
    ? MASTER_DIRECTORY_DATA.find(x=>String(x.id)===String(unitId))
    : null;

  $("masterUnitId").value=unit?.id ?? "";
  $("masterUnitCode").value=unit?.code ?? "";
  $("masterUnitName").value=unit?.name ?? "";
  $("masterUnitAddress").value=unit?.address ?? "";
  $("masterUnitHead").value=unit?.unit_head ?? "";
  $("masterUnitManager").value=unit?.manager_ffb ?? "";
  $("masterUnitActive").checked=unit ? !!unit.active : true;

  const editing=!!unit;
  $("masterUnitModalTitle").textContent=editing ? `Edit Kantor Unit ${unit.code}` : "Tambah Kantor Unit";
  $("masterUnitModalHint").textContent=editing
    ? "Perbarui nama, alamat, penanggung jawab, atau status unit."
    : "Tambahkan unit baru agar dapat digunakan pada modul operasional.";
  $("masterUnitSaveBtn").textContent=editing ? "Simpan Perubahan" : "Simpan Kantor Unit";

  // Kode dikunci saat edit agar histori transaksi dengan kp_code lama tidak terputus.
  $("masterUnitCode").disabled=editing;
  $("masterUnitCodeHelp").textContent=editing
    ? "Kode KP dikunci saat edit untuk menjaga konsistensi histori tonase, harga, dan pengeluaran."
    : "Kode harus unik. Huruf otomatis menjadi kapital.";

  setMasterUnitFormMessage("");
  modal.classList.remove("hidden");
  modal.setAttribute("aria-hidden","false");
  document.body.classList.add("modal-open");

  setTimeout(()=>$(editing?"masterUnitName":"masterUnitCode")?.focus(),50);
}
...
async function validateMasterUnitCode(code,currentId=null){
  const normalized=normalizeMasterUnitCode(code);
  if(!normalized) return {ok:false,message:"Kode KP wajib diisi."};

  if(!/^[A-Z0-9]+(?:-[A-Z0-9]+)*$/.test(normalized)){
    return {ok:false,message:"Format kode KP tidak valid. Gunakan huruf, angka, dan tanda -."};
  }

  const localDuplicate=MASTER_DIRECTORY_DATA.find(x=>
    String(x.code||"").toUpperCase()===normalized &&
    String(x.id)!==String(currentId||"")
  );
  if(localDuplicate){
    return {ok:false,message:`Kode KP ${normalized} sudah digunakan oleh ${localDuplicate.name||localDuplicate.code}.`};
  }

  const {data,error}=await db.from("master_kp")
    .select("id,code,name")
    .eq("code",normalized)
    .limit(1);

  if(error){
    console.warn("Validasi kode KP ke database gagal:",error);
    return {ok:false,message:"Validasi kode KP ke database gagal. Coba lagi."};
  }

  const dbDuplicate=(data||[]).find(x=>String(x.id)!==String(currentId||""));
  if(dbDuplicate){
    return {ok:false,message:`Kode KP ${normalized} sudah ada di database.`};
  }
...
async function saveMasterUnit(){
  const saveBtn=$("masterUnitSaveBtn");
  const id=$("masterUnitId").value || null;
  const isEdit=!!id;

  const name=$("masterUnitName").value.trim();
  const address=$("masterUnitAddress").value.trim();
  const unitHead=$("masterUnitHead").value.trim();
  const manager=$("masterUnitManager").value.trim();
  const active=$("masterUnitActive").checked;

  const rawCode=$("masterUnitCode").value;
  const existing=isEdit
    ? MASTER_DIRECTORY_DATA.find(x=>String(x.id)===String(id))
    : null;
  const candidateCode=isEdit ? existing?.code : rawCode;

  if(!name){
    setMasterUnitFormMessage("Nama Kantor / Perusahaan wajib diisi.","error");
    $("masterUnitName").focus();
    return;
  }

  setMasterUnitFormMessage("Memeriksa kode KP...","working");
  saveBtn.disabled=true;

  try{
    const codeCheck=await validateMasterUnitCode(candidateCode,id);
    if(!codeCheck.ok){
      setMasterUnitFormMessage(codeCheck.message,"error");
      return;
    }
    ...
    const payload={
      ...
      unit_head:unitHead||null,
      manager_ffb:manager||null,
      active
    };

    let result;
    if(isEdit){
      // Kode tidak diubah pada mode edit.
      delete payload.code;
      result=await db.from("master_kp").update(payload).eq("id",id).select("id,code,name").single();
    }else{
      result=await db.from("master_kp").insert(payload).select("id,code,name").single();
    }

    if(result.error){
      if(result.error.code==="23505" || /duplicate|unique/i.test(result.error.message||"")){
        setMasterUnitFormMessage(`Kode KP ${codeCheck.code} sudah digunakan. Gunakan kode lain.`,"error");
      }else{
        setMasterUnitFormMessage("Gagal menyimpan: "+result.error.message,"error");
      }
      return;
    }

    setMasterUnitFormMessage(isEdit?"Perubahan berhasil disimpan.":"Kantor unit berhasil ditambahkan.","success");

    await loadMaster();

    setTimeout(()=>{
      closeMasterUnitModal();
      if($("masterSearch")) $("masterSearch").value=result.data?.code||"";
      renderMasterDirectory();
    },450);

  }catch(err){
    console.error(err);
    setMasterUnitFormMessage("Terjadi kesalahan saat menyimpan kantor unit.","error");
  }finally{
...
document.addEventListener("keydown",e=>{
  if(e.key==="Escape" && !$("masterUnitModal")?.classList.contains("hidden")){
    closeMasterUnitModal();
  }
});
    ...
    <div class="master-unit-card">
      <div class="master-unit-card-head">
        ...
        <div class="master-card-actions">
          ...
          <button class="master-edit-btn" type="button" onclick="openMasterUnitModal('${u.id}')">Edit</button>
        </div>
      </div>
